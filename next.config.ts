...
// all live. See headers() emit block below for the actual config. CSP
// will flip from Report-Only to enforce after 1-2 week observation window.
...
const CSP_REPORT_ONLY =
  ...
  "connect-src 'self' https://brapp.greenlifecannabis.com " +
...
const nextConfig: NextConfig = {
  images: {
    ...
    // _NEAR_PAGES_2026_05_14.md — sister scc v26.705 same-push.
  ...
  async headers() {
    return [
      ...
      // these paths interact with /menu or iHeartJane Boost — safe.
      {
        ...
        // class). Layered header bypasses the strip. Sister scc + GW Z48.
        source: "/sitemap.xml",
        headers: [
      ...
      },
      {
        ...
        // risk; 24× fewer crawler-driven cold edge rebuilds. Sister scc.
      ...
      {
        // Same rationale as /robots.txt — rebuilds on deploy. Sister scc.
        // Vercel-CDN-Cache-Control layered 2026-05-15 (GW Z48 port).
        source: "/llms.txt",
        headers: [
          { key: "Cache-Control", value: "public, max-age=86400, s-maxage=86400" },
      ...
      },
      {
        ...
        // deploy, so 24hr edge cache is safe. Sister scc + GW Z48 port.
  ...
  async redirects() {
    return [
      ...
      // convention `app/apple-icon.png/route.tsx`). Sister scc same-fix.
