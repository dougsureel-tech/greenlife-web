...
|---|---|---|---|
| 2026-05-19 | v37.645 | 6b10319 | 🎨 /menu-preview cycle-4 polish — sister-port of scc v29.005 + v29.025 BUNDLED into single GLW push (build cadence: catch up both SCC rounds in 1 deploy). Six items: (1) **`ProductImage` brand-logo fallback** — products with no `image_url` but known `brand` render the wordmark from `/public/brand-logos/` (60+ logos shipped). `onError` falls to category icon for brands without a logo file — no allowlist, no broken-image risk. Brand pill suppressed when logo renders. (2) **DOH chip stacked** — `top-12` when deal-pill present, `top-2.5` solo. Swapped 🟣 emoji for inline SVG medical cross. `<TODO swap inline SVG>` comment for the official WA-DOH logo (Doug to re-drop). (3) **Strain chip always bottom-right** — was conditional top-left when no deal which would collide with new DOH placement on DOH+strain cards. Bottom edge now symmetric weight-left / strain-right. (4) **Empty-state next-steps overhaul** — bolder copy, pill-button CTAs, NEW category-pivot row with top-4 in-stock categories as one-tap recovery buttons. Search-miss path gets distinct copy + phone-the-store fallback. (5) **Hero contrast bump** — `text-green-300/70` + `text-green-200/70` → `text-green-100` + `text-green-200/90` — clears WCAG AA on slate-900. (6) **Responsive grid gap** — `gap-3 sm:gap-4`. 2 files: `app/order/OrderMenu.tsx` (~125 LOC) + `app/menu-preview/page.tsx` (2-line) + version.ts. typecheck CLEAN. Pre-commit Explore CLEAN. 370/370 tests passing. Verify `v37.645 / sha=6b10319 / ok=true`. |
