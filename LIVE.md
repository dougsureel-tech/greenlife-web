...
|---|---|---|---|
| 2026-06-13 | **v43.206** | b6f3545 | 🛡️ WSLCB accuracy-fix: removed the LIVE FALSE "closest legal cannabis to the lake" claim on `/near/chelan` (Bridgeport ~19.9mi / Brewster ~20.5mi are closer; GL ~29.4mi) — reframed to the true route/occasion angle per `SEO_CHELAN_OPPORTUNITY_2026_06_13.md` §4 (same class as the Manson v33.905 fix). Repointed the legacy Google-indexed `/chelan-dispensary` 308 from `/menu` → `/near/chelan`. Verify-via-curl `v43.206 / sha=b6f3545 / ok=true`; live `/near/chelan` has 0 "closest" + the reframe present; `/chelan-dispensary` 308→/near/chelan→200. 1968/1968 tests, all WSLCB+brand-voice gates PASS, pre-push Explore CLEAN. Doug greenlit 2026-06-13. |
| 2026-06-13 | **v43.196** | 45ac447 | 🏬 NEW `/wenatchee-dispensary` head-term landing page (was 404 → 200), WSLCB-gated. |
| 2026-06-12 | **v43.186** | 90aa090 | 🛡️ WSLCB efficacy-FRAME sister-port — 3 frame regexes (good/great/perfect-for · knocks-out · calms-you/your) added to `scrubWslcbClaims` in `lib/strain-product-json-ld.ts` (frame-only, won't mangle "Calm Tincture"); all 3 copies (inv-App + glw + scc) now in sync. Coupled gate-fix: `check-csp-report-only.mjs` now accepts CSP **enforce** mode (GLW graduated at v43.065) — the stale gate had been false-failing every push, NOT missing infra. Verify-via-curl `v43.186 / sha=90aa090 / ok=true`. 1968/1968 tests. |
| 2026-06-09 | **v43.135** | 97088ed | 🛒 Native-menu `?category=` deep-link resolver + true Vapes family group (sister scc v34.635). `resolveCategoryParam()` maps homepage tile slugs to in-stock canonical buckets (never empty grid); `CATEGORY_GROUPS.Vapes` spans Cartridges+Disposables+Pods via `categoryMatches()` (Doug 2026-06-09 "all in one = disposable"). Dormant until `NATIVE_MENU_LIVE` flips. Verify-via-curl `v43.135 / sha=97088ed / ok=true`. tsc clean. Pre-push Explore CLEAN. |
| 2026-05-30 | **v42.745** | 2d02bbf | 🛍️ **PDP preview surface `/menu/preview/[id]` — Phase 0 of Product UX Redesign (sister scc v34.045).** New Curaleaf-hierarchy product detail page at noindex preview URL — hero image OR brand-fallback specimen, brand small-caps, oversized THC% (text-5xl green-800), effect chips via keyword match on product `notes`/`effects` (WAC 314-55-155 preference-context register), terpene-dominance pills, price + "Out the door: $X" (1.458× shelf for Wen 37%+8.8%), full-width Add-to-cart CTA wiring to existing `gl_cart` localStorage + `/menu-preview?cart=open` drawer-open mechanism, verified-purchaser reviews section (hidden when count=0 per UX brief). 7 NEW pure helpers at `lib/pdp-helpers.ts` (44 pin tests). NEW `getPreviewProductBundle` DB fn with brand-recent-sales gate + floor-only two-bucket discipline + `product_reviews` published-only filter + `customers` join for tenure label. **Re-bump from v42.725:** the sister-session v42.705 commit accidentally bundled my v42.725 changelog text without the code; v42.745 lands the actual code. Click-test: visit `/menu/preview/<product-id>` directly (not via /menu link — none added per Phase 0 scope). |
|---|---|---|---|
