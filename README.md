...
Every `git push` runs `.githooks/pre-push` (30 gates total). The 7
...
| `check-no-unsafe-redirect` | v30.405 (sister inv v397.485) — preventive: `searchParams.get("returnTo")` → `router.push(target)` open-redirect class lock |
