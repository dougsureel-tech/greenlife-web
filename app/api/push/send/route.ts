...
// anyway. Opt out for this intentional pattern.
...
export async function POST(req: NextRequest) {
  ...
  // SEND_TOKEN byte-by-byte. Same defense pattern as scc verify-code
  // (v6.105) + inv cloudprnt (v186.845) + lib/rewards-session.ts.
  // Length-mismatch short-circuits because timingSafeEqual throws on
  // differing buffer lengths and length isn't a secret.
  ...
  // Fire in parallel; web-push on each subscription is independent. Push providers
  // (FCM, Mozilla, Apple) all rate-limit individually so this scales fine for our
  // size of customer base (low thousands at most).
  await Promise.all(
    subs.map(async (s) => {
      try {
        ...
        if (status === 404 || status === 410) {
          ...
          // Status code is fine to log; truncated endpoint host is fine
          // for debugging; the err object itself is not. Sister of v7.425.
