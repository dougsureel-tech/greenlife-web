...
const LINKS: DevLink[] = [
  { href: "/devmenu", label: "In-tree menu (WIP)", blurb: "The custom menu we're building to replace iHeartJane. Sidebar, filters, parsed titles.", status: "wip" },
  { href: "/order",   label: "Order flow",          blurb: "Same in-tree menu component, but the live customer-facing route + cart + pickup time.", status: "wip" },
  { href: "/menu",    label: "iHeartJane embed",    blurb: "What customers actually see right now. Comparison reference for the in-tree menu.", status: "live" },
  { href: "/stash",   label: "Stash",               blurb: "Customer-saved-products page (localStorage hydrated to full cards).", status: "live" },
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10 space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-stone-500 text-sm">
          Bookmark this page. Quick links to work-in-progress and reference routes — works against whichever port the dev server picked.
        </p>
      </div>
        ...
        {LINKS.map((l) => (
          <Link key={l.href} href={l.href}
            ...
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border ${STATUS_BADGE[l.status]}`}>
                {l.status}
              </span>
