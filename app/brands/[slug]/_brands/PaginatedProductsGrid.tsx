...
type Product = {
  ...
  // Optional — when upstream query SELECTs it, DohLogo renders High THC /
  // High CBD / General Use per WAC 246-70. When absent (current state),
  // defaults to General Use which is the DOH framework's catch-all.
...
export function PaginatedProductsGrid({
  ...
  products: Product[];
  /** Active deals so each card can apply BIGGER of (20% online floor, daily-deal %).
   *  Defaults to empty → only the 20% floor applies. Brand-override pages should
   *  pass the deals fetched at the top-level brand page (BrandComponentProps). */
  deals?: ActiveDeal[];
  accentBg?: string;
  accentBorder?: string;
                ...
                return (
                  ...
                  >
                    {p.image_url ? (
                      ...
                      // placeholder + brand chip. h-44 to match the image case so
                      // grid rows stay aligned (pre-v36.405 was h-32 which jagged
                      // the row when a row mixed photo and no-photo cards).
                      <div
                        role="img"
                        aria-label={p.name}
            ...
            {(() => {
              ...
              return (
                <div className="flex flex-col leading-tight">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-wider text-green-700 leading-none mt-0.5">
                    {pricing.dealName ? `${Math.round(pricing.discountPct)}% off · ${pricing.dealName}` : `${ONLINE_DISCOUNT_PCT}% off online`}
                  </span>
                </div>
              );
