    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10 space-y-6">
      ...
      {orders.length === 0 ? (
        <div className="rounded-2xl border border-stone-200 bg-white px-8 py-16 text-center space-y-4 shadow-sm">
          ...
          <Link href="/order" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-green-700 text-white text-sm font-semibold hover:bg-green-600 transition-all hover:-translate-y-0.5 shadow-sm">
            Browse Menu →
          </Link>
        </div>
      ) : (
          ...
          {orders.map((order) => {
            ...
            return (
              <div key={order.id} className={`rounded-2xl border bg-white overflow-hidden shadow-sm ${
                isReady ? "border-green-300 ring-2 ring-green-200/40" : "border-stone-200"
              }`}>
                {/* Order header row */}
                ...
                {/* Ready callout */}
                {isReady && (
                  <div className="px-5 py-4 bg-gradient-to-br from-green-50 to-emerald-50 border-b border-green-100">
                    <div className="flex items-start gap-3">
                      <div className="w-9 h-9 rounded-full bg-green-600/10 flex items-center justify-center text-green-700 text-lg shrink-0">✓</div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-green-900">Ready for pickup{order.readyAt ? ` since ${fmtTime(order.readyAt)}` : ""}.</p>
                        <p className="text-xs text-green-700/80 mt-0.5">Bring your ID and cash. Show this order at the counter.</p>
                        <div className="flex flex-wrap gap-2 mt-3">
                          <a href={STORE.googleMapsUrl} target="_blank" rel="noopener" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white border border-green-200 hover:border-green-300 text-green-800 text-xs font-semibold transition-colors">
                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                            Get directions
                          </a>
                          <a href={`tel:${STORE.phoneTel}`} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white border border-green-200 hover:border-green-300 text-green-800 text-xs font-semibold transition-colors">
                ...
                {isActive && (
                  <Link href={`/order/confirmation/${order.id}`} className="block px-5 py-2.5 border-t border-stone-100 text-xs text-stone-500 hover:text-green-700 hover:bg-stone-50 transition-colors text-center font-medium">
                    View order details →
                  </Link>
                )}
              </div>
