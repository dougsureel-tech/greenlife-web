...
export function StrainFinderClient() {
  ...
  return (
    <div className="space-y-6">
        ...
        {step.options.map((opt) => {
          ...
          return (
            <button
              ...
              disabled={submitting}
              className={`group relative rounded-2xl border p-4 sm:p-5 text-left transition-all hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-green-500 disabled:opacity-50 ${
                selected
            ...
            >
              ...
              <div className={`text-sm font-semibold transition-colors ${selected ? "text-green-800" : "text-stone-900 group-hover:text-green-700"}`}>
                {opt.label}
              </div>
              {opt.sub && (
      ...
      {submitting && (
        <p className="text-center text-sm text-green-700 font-semibold animate-pulse">Finding your matches…</p>
      )}
    </div>
  );
}
