...
export default async function HomePage() {
  ...
  const [brands, featured, justIn, deals, closure, treasureChest] = await Promise.all([
    ...
    // CDN-cache fix: pass `revalidate` so the upstream closure-status fetch
    // does NOT use `cache: "no-store"` (the default), which would opt the
    // whole homepage out of ISR despite the `revalidate=60` export. 60s
    // matches the page's revalidate window.
  ...
  // each rendered as a single-letter initial placeholder — a wall of "A" / "B"
  // cards with no logos, the opposite of "Washington's finest producers." The
  // existing initials fallback stays for any single brand on /brands/[slug]
  // that's logo-less, but the homepage carousel is a curated brand showcase:
  // no logo → no slot. `featuredBrands.length > 0` gate below hides the whole
  // section when the filter returns empty.
  // Logo source preference (Doug 2026-05-17 — Fix A from backfill audit):
  // Prefer self-hosted `/brand-logos/<slug>.png` (curated, known-good,
  // high-contrast white-bg PNG) over DB external URLs (often point at
  // brand-own-site CDNs that 404 or return faded/transparent logos —
  // e.g. EVERGREEN HERBAL's DB row pointed at the420bar.com, an
  // unrelated cannabis-news blog). Matches the fallback chain on
  // /brands/[slug] (`dbLogoUrl ?? getBrandCopy(slug)?.logoUrl`). Pre-
  // fix, this carousel only read DB and surfaced the bad URLs while
  // ignoring our 28 known-good self-hosted PNGs.
  ...
  return (
    <>
      ...
      {/* ─── Active deals strip — only renders when something's actually
            ...
            so a customer sees "20% off Flower today" alongside "what's
            good?", instead of needing to dig into /deals. Source of truth
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
            <div className="flex items-end justify-between gap-3 mb-5 flex-wrap">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <Link
                  href="/deals"
                  className="text-sm font-semibold text-green-700 hover:text-green-600 transition-colors whitespace-nowrap"
                >
                  All deals →
                </Link>
              </div>
              ...
              {deals.slice(0, 3).map((d) => {
                ...
                return (
                  ...
                  >
                    ...
                    <h3 className="font-bold text-stone-900 text-base mt-3 group-hover:text-green-800 transition-colors">
                      {d.name}
                    </h3>
                    {d.description && (
      ...
      {/* ─── Hours + Map ────────────────────────────────────────────────────── */}
      <section className="bg-white border-b border-stone-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
      ...
      <section className="bg-green-950 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-10 flex flex-col sm:flex-row items-center justify-between gap-8">
  ...
  const faqs: { q: string; a: string }[] = [
    {
      ...
      a: "Green Life Cannabis is at 3012 GS Center Road Ste A in Wenatchee — the Sunnyslope side of town, closest dispensary to Lake Chelan, the orchards, and US-2 west into the Cascades. Open every day from 8 AM, with Friday and Saturday hours running later. We carry 100+ Washington brands, all WSLCB-licensed.",
    },
    {
